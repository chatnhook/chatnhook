...
class InspectionCreatedEvent(BaseEvent):
    def process(self):
        ...
        if 'inspection.created' not in self.config['events']:
            return False

        inspection = self.body['uuid'].split('-')[-1]
        inspection_link = 'https://scrutinizer-ci.com' + self.body['_links']['self']['href'].replace('/api/repositories', '')
        ...
        message = '⚒ Inspection [{inspection}]({inspection_url}) *created* for {repository}@{branch}\n' \
            ...
            repository=self.body['_embedded']['repository']['login'] + '/' + self.body['_embedded']['repository']['name'],
