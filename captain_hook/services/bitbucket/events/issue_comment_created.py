...
class IssueCommentCreatedEvent(BitbucketEvent):
    def process(self, request, body):
        comment = body.get('comment', {})
        issue = body.get('issue', {})
        comment_api_link = str(comment.get('links', {}).get('self', {}).get('href')).replace(
            'https://api.bitbucket.org/2.0/', '')
        issue_api_link = str(issue.get('links', {}).get('self', {}).get('href')).replace(
            'https://api.bitbucket.org/2.0/', '')
        ...
        message = "[🗨]({comment_link}) [{username}]({user_link}) commented " \
                  "on issue [#{issue_number} {issue_title}]({comment_link})"
    ...
    def get_redirect(self, request, event, params):
        ...
        title = '{username} replied · {issue_title} · {issue_type} #{issue_number} · {repo}'.format(
            issue_title=issue.get('title').encode('utf-8'), issue_number=str(issue.get('id')),
            repo=repo,
