...
class IncidentAcknowledgeEvent(BaseEvent):
    def process(self):
        ...
        acknowledger = payload['data']['incident']['acknowledgers'][0]['object']
        incident = payload['data']['incident']
        message = '[{name}]({user_link}) has acknowledged incident [#{incident_number} {incident_title}]({incident_link})'.format(
